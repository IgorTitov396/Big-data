...
object Main extends App {
  ...
  class TokenizerMapper extends MapperType {
    override def map(key: AnyRef, value: Text, context: MapperType#Context): Unit = {
      Try {
        ...
        context.write(new OutputKeyMapperClass(id), new OutputValueMapperClass(Utils.stringPairAsText(domain, rawData.toString)))
      }.getOrElse {
  ...
  class MyReducer extends ReducerType {
    override def reduce(key: OutputKeyMapperClass, values: lang.Iterable[OutputValueMapperClass], context: ReducerType#Context): Unit = {
      Try {
        val seq = values.iterator().toList

        def trimTodd(str: String): String = {
          str.take(10)
        }

        val count = seq.map(Utils.textAsStringPair)
          .map(v => v._1 + TestUtils.separator + trimTodd(v._2))
          .distinct
          .map(TestUtils.textAsStringPair)
          .groupBy(_._2)
          .mapValues(_.map(_._1).distinct)
          .values.maxBy(_.length)

        context.write(new OutputKeyReducerClass(key.toString + ":"), new OutputValueReducerClass( "[" + count.mkString(",") + "]"))
      }.getOrElse(())
    }
